...
<Note type="legacy">
  **Lưu ý**: This is an old blog post.
  The below is kept as is for historical purposes.
</Note>

<Note type="info">
  **Lưu ý**: Info on how to migrate is available in our
...
## Improvements to the Định dạng MDX
...
We believe it’s more intuitive that markdown “inlines” chẳng hạn như emphasis can form
between tags on the same line (first `<div>`), “blocks” chẳng hạn như headings can
...
just work: import `readme.mdx` and it’s seen as the Định dạng MDX, import
`readme.md` and it’s seen as markdown, based on their extensions.

The Định dạng MDX is described in [§ What is MDX?][what]
...
Bạn có thể even import MDX files directly in Node.js with our new
`@mdx-js/node-loader`.
...
See [§ Bắt đầu][getting-started] for a quick start but also in-depth
info on how to integrate MDX with many different tools.

## Architectural improvements

To make the syntax of the Định dạng MDX better and to allow new integrations and
...
It also means that we have a new AST which mô tả the Cú pháp MDX in more
detail — we even expose the embedded JavaScript.
This detailed AST cho phép plugins to enhance MDX in powerful new ways.
...
works.
See [§ Mở rộng MDX][extending] for several plugins that use the new tree to
...
Bạn có thể use it, if you’re importing MDX files, to type those imported
components.

See [¶ TypeScript in § Bắt đầu][ts] for more on how to use configure
TypeScript.
...
tools chẳng hạn như Lighthouse and axe.

See [§ Đóng góp][contribute] for more on how to help.
