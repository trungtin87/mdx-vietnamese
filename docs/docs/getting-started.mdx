...
Bài viết này giải thích how to integrate MDX into your project.
It cho thấy how to use MDX with your bundler and JSX runtime of choice. {/* more */}
To understand how the Định dạng MDX works,
we recommend that you start with [§ What is MDX][what].
See [§ Sử dụng MDX][use] when you’re all set up and ready to use MDX.
...
MDX relies on JSX,
so it’s required that your project hỗ trợ JSX cũng như.
Any JSX runtime (React, Preact, Vue, etc.) will do.
Lưu ý that we do compile JSX to JavaScript for you so you don’t have to set that
up.
...
<Note type="info">
  **Lưu ý**: Using Rust instead of Node.js?
...
Bạn có thể also use MDX without bundlers:
...
* if you use **Theme UI**,
  ...
  then wrap your Nội dung MDX in a `<ThemeProvider />`
...
Bạn có thể enhance the experience of using MDX by adding support of it to your
editor:
...
chẳng hạn như [`@types/react`][definitely-typed-react],
...
chẳng hạn như `MDXComponents` which represents the `components` prop.
Bạn có thể import them như sau:
...
Xem thêm [¶ Bun][javascript-engines-bun],
...
Xem thêm [¶ Vite][build-system-vite],
...
Xem thêm [¶ Next.js][site-generator-next],
...
<Note type="info">
  **Lưu ý**: If you also use `@vitejs/plugin-react`,
...
Xem thêm [¶ Rollup][bundler-rollup] which is used in Vite and see
...
<details>
  ...
  …can be used như sau with the Babel API:
...
Bạn có thể add the integration with the Astro CLI: `npx astro add mdx`.
...
[Docusaurus][] hỗ trợ MDX theo mặc định.
...
<Note type="info">
  **Lưu ý**: the official Parcel plugin is currently not maintained.
...
`'@emotion/react'`.
Bạn có thể optionally install and configure [`@mdx-js/react`][mdx-react] to
support context based component passing.

Xem thêm [¶ React][jsx-runtime-react],
...
Xem thêm [¶ Node.js][js-engine-node] and [¶ React][jsx-runtime-react] for more
info.
...
Bạn có thể optionally install and configure [`@mdx-js/preact`][mdx-preact] to
support context based component passing.

Xem thêm [¶ Rollup][bundler-rollup], [¶ esbuild][bundler-esbuild], and
[¶ webpack][bundler-webpack],
which you might be using,
for more info.
...
React is supported theo mặc định.
Bạn có thể optionally install and configure [`@mdx-js/react`][mdx-react] to
support context based component passing.
...
Bạn có thể optionally install and configure [`@mdx-js/vue`][mdx-vue] to
support context based component passing.
...
Xem thêm [¶ Rollup][bundler-rollup] and [¶ Vite][build-system-vite],
which you might be using,
for more info.
...
* If you want to use Nội dung MDX in your project,
  see [§ Sử dụng MDX][use]
* If you’re getting errors integrating MDX,
  see [§ Khắc phục sự cố MDX][trouble] or [§ Hỗ trợ][support]
