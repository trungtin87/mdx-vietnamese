...
<Note type="info">
  **Lưu ý**: Had trouble with something that wasn’t explained here but should be?
  Please let us know.
  See [§ Đóng góp][contribute] for how to help.
</Note>
...
when using MDX. {/* more */}
To understand how the Định dạng MDX works, we recommend that you start with
[§ What is MDX][what].
How to use [our packages][packages] is documented in their readmes.
To migrate to the latest MDX, see [§ Di chuyển from v1 to v2][migation-v2].
...
which giải thích in detail how to use ESM with many different tools.
...
If you’re having trouble with tools that don’t support ESM, chẳng hạn như Electron,
...
MDX file.
This was typically used to support frameworks other than React chẳng hạn như Preact.
We now have options chẳng hạn như `jsxImportSource` for that and arbitrary JavaScript
...
Please see [¶ Preact in § Bắt đầu](/docs/getting-started/#preact) for
how to support Preact.
See [¶ Tạo plugins in § Mở rộng MDX](/docs/extending-mdx/#creating-plugins)
...
The full error message in MDX 2 is như sau:
...
The `format` option, when configured with `'detect'`, cho phép inferring whether a
...
This is a warning.
It is not an error.
Bạn có thể keep on using these options, but expect them to be removed in the
future.
...
`evaluate` hỗ trợ React and other frameworks.
...
The full error message in MDX is như sau:
...
* **Incorrect interleaving**
  — See the rules in
  [¶ Xen kẽ in § What is MDX?][interleaving]
* **Broken JavaScript**
  — Đảm bảo the JavaScript you write is valid
...
is an expression.
That means statements (chẳng hạn như `if` and `else` and `for` loops) do not work.
...
The reason for this error is that JSX only cho phép spreading a single value at a
time:
...
The reason for this error is that JSX only cho phép spreading values:
...
The reason for these errors is that JSX has a very strict grammar and mong đợi
tags to be valid.
There are different solutions depending on what was expected.
Vui lòng đọc the error message carefully as it indicates where the problem
