...
Bài viết này giải thích how to use MDX files in your project.
It cho thấy how you can pass props and how to import, define, or pass components. {/* more */}
See [§ Bắt đầu][start] for how to integrate MDX into your project.
...
An integration compiles Cú pháp MDX to JavaScript.
...
Bạn có thể also use it with [Preact][],
[Vue][], [Emotion][],
[Theme UI][],
etc.
...
Bạn có thể use those components like any other component in your framework of
choice.
Take this file:
...
It could be imported and used in a React app như sau:
...
It can be imported from JavaScript and passed components như sau:
...
Bạn có thể also define or import them within MDX:
...
* HTML equivalents for the things you write with markdown chẳng hạn như `h1` for
...
name (like `h1`) or not (like `Component`) are như sau:
...
references is illustrated như sau.
...
Context cung cấp a way to pass data through the component tree without having to
...
3. Import `MDXProvider` from that package.
   Use it to wrap your top-most Nội dung MDX component and pass it your
   `components` instead:
