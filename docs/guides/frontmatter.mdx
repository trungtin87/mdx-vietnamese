...
That means frontmatter is not supported theo mặc định.
...
Then without compiling or evaluating the metadata can be accessed như sau:
...
Our compiler, `@mdx-js/mdx`, doesn’t understand YAML frontmatter theo mặc định but
