...
Hướng dẫn này cho thấy how to use `@mdx-js/mdx` to compile MDX on the server and run
the result on clients. {/* more */}
Some frameworks, chẳng hạn như Next.js and Remix, make it easy to split work between
servers and clients.
...
[`next-mdx-remote`][next-mdx-remote] for, but MDX also hỗ trợ it.
...
<Note type="info">
  **Lưu ý**: MDX is not a bundler (esbuild, webpack, and Rollup are bundlers):
...
Some frameworks let you write the server and client code in one file, chẳng hạn như
Next.
