...
blocks. {/* more */}
MDX hỗ trợ standard markdown syntax ([CommonMark][]).
It does not apply syntax highlighting to code blocks theo mặc định.
...
<Note type="important">
  **Quan trọng**: you must likely also include CSS somewhere on the page.
  See the documentation of the plugin you’re using để biết thêm thông tin.
</Note>
...
Markdown hỗ trợ a meta string for code:
...
The `meta` part is everything after the language (trong trường hợp này, `js`).
This is a *hidden* part of markdown: it’s normally ignored.
But as the above example cho thấy, it’s a useful place to put some extra fields.
...
More info on plugins is available in [§ Mở rộng MDX][extend]
