...
<Note type="legacy">
  **Lưu ý**: This is an old migration guide.
  See [§ Di chuyển from v2 to v3](/migrating/v3/).
  The below is kept as is for historical purposes.
</Note>

# Di chuyển from v1 to v2
...
Please see [¶ ESM in § Khắc phục sự cố MDX][trouble-esm].
...
If you’re experiencing problems, please see [§ Khắc phục sự cố MDX][trouble]
...
Bạn có thể update your code như sau:
...
* [`@mdx-js/react`][mdx-react] is optional.
  ...
  More info in [¶ MDX provider in § Sử dụng MDX][mdx-provider]

`@mdx-js/loader` now hỗ trợ Preact and other JSX runtimes through
configuration.
...
để biết thêm thông tin, please see [§ API in `@mdx-js/loader`][loader-api].
...
Lưu ý that these packages now only add support for context based components
passing.
...
* The named export `mdx`, which was a `createElement` function, is removed.
  Bạn có thể use your framework’s functions instead: `React.createElement`,
  `h` from `preact`, etc.
...
`@mdx-js/mdx` now hỗ trợ Preact and other JSX runtimes through configuration.
Using Preact as an example:
...
để biết thêm thông tin, please see [§ API in `@mdx-js/mdx`][mdx-api]
...
để biết thêm thông tin, please see [`evaluate` in `@mdx-js/mdx`][mdx-evaluate].

###### Changes

* The package `@mdx-js/runtime` is replaced by [`evaluate`][mdx-evaluate]
  from `@mdx-js/mdx`
* `evaluate` hỗ trợ any JSX runtime and providers are optional
* `evaluate` yields an `MDXContent` component just like how importing
  and MDX file works
* `evaluate` hỗ trợ imports and exports inside evaluated MDX
...
để biết thêm thông tin, please see [§ Use in `remark-mdx`][remark-mdx-use].
...
Then, see [¶ Vue in § Bắt đầu](/docs/getting-started/#vue) on how to use
Vue with MDX.
...
Then, see [¶ Parcel in § Bắt đầu](/docs/getting-started/#parcel) on how
...
In version 2, we improved the syntax of the Định dạng MDX.
...
See [§ Khắc phục sự cố MDX][trouble] for common errors and how to solve them.
...
Bạn có thể now indent your JSX and markdown:
...
Bạn có thể use markdown “inlines” but not “blocks” inside JSX if the text and tags
...
So trong trường hợp này two paragraphs and a heading.
...
Bạn có thể now use expressions in MDX to include JavaScript.
Bạn có thể also use them as an escape hatch if you ever *just* want strings or JSX
...
[¶ Biểu thức in § What is MDX?][what-expressions]

### ESM

Bạn có thể more easily embed components in MDX because blank lines are allowed:
...
We turned off GFM features in MDX theo mặc định.
...
* `parent.child` combos chẳng hạn như `ol.li`, to pass components, were removed, we
...
* Bạn có thể now pass and use objects of components: if you pass
